...
contract OffchainAggregator is Owned, OffchainAggregatorBilling, AggregatorV2V3Interface, TypeAndVersionInterface {
  ...
  uint debug_remainingLink;
  uint debug_totalLINKDue;
   ...
   */
  function requestNewRound(address _requesterAddr) external returns (uint80) {
    require(maxRequestLinkCost>0, "_maxRequestLinkCost not set");
  ...
  {
    /////////////////////////////////////////////////////////////////////////////////////////  
    uint64 currJob = tracker.currReq();
    require(currJob > 0, "report not needed, no requests in queue");
    /////////////////////////////////////////////////////////////////////////////////////////  
    ...
    { // Check the report contents, and record the result
      ...
      /////////////////////////////////////////////////////////////////////////////////////////
      int192 median = getMostFrequentValue(r.observations, r.hotVars.threshold, currJob);
      /////////////////////////////////////////////////////////////////////////////////////////
    ...
    if(linkPayed<maxRequestLinkCost)
  ...
  function setMaxrequestLinkTonken(uint _maxRequestLinkCost)
  public
  onlyOwner()
  {
    require(_maxRequestLinkCost>0, "_maxRequestLinkCost must be grather than zero");
    maxRequestLinkCost = _maxRequestLinkCost;
  }

  //returns the most frequent value for the current job and makes sure
  //that that value is supported by at least f observations
  function getMostFrequentValue(int192[] memory observations, uint f, uint64 currJob)
  internal
  returns(int192)
  {
    int192[] memory filteredObservation = observations;
    uint c = 0;
    uint64 id;
    for(uint i=0; i<observations.length; i++){
      id = uint64( observations[i] >> 128);
      if(id==currJob){
          filteredObservation[c] = observations[i];
          c++;
      }
    }
    require(c > f, "not enough signatures have been submitted for the current jobID");

    int192 value;
    uint maxFrequency = 0;
    uint tempFrequency;
    for(uint i=0; i<c; i++){ //N^2 do sort+scan
      tempFrequency = 0;
      for(uint k=0; k<c; k++){
        if(observations[k]==observations[i])
          tempFrequency++;
      }
    ...
    require(maxFrequency > f, "not enough signatures have agreed on the same value");
    return value;
  }
}
