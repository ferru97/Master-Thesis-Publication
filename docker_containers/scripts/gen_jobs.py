...
observationSource = """
    // data source 1
    eth_spent_hash          [type=http method=GET url="http://10.5.0.6:3000/getDataHash?reqmanager={}&oid={}"]
...
observationSource = """
    ...
    fun_calls    [type=http method=POST url="http://10.5.0.6:3000/getSavedData" requestData="{ \\\\"hash\\\\": $(decode_cbor.hash), \\\\"rid\\\\": $(decode_cbor.rid), \\\\"oid\\\\": \\\\"-tra-\\\\"}" allowUnrestrictedNetworkAccess=true]
    ...
    encode_mwr [type="ethabiencode"
                ...
                data="{\\\\"requestID\\\\": $(decode_log.requestId), \\\\"functionCalls\\\\": $(fun_calls_json), \\\\"pastValue\\\\": $(past_value_json), \\\\"sender\\\\": \\\\"-tra-\\\\", \\\\"queueID\\\\": $(decode_cbor.rid)}"]
...
def getConf():
    ...
    conf["manager"] = (lines[5].split("="))[1].strip()
    conf["dr"] = (lines[6].split("="))[1].strip()
    conf["boots_peer"] = (lines[7].split("="))[1].strip()
    conf["boot_kb"] = (lines[8].split("="))[1].strip()
    return conf
...
def main():
    ...
    for i in range(0,len(conf["transmit_address"])):
        ocr_job = job.format(conf["ocr"].strip(),conf["p2p_id"][i].strip(),conf["boots_peer"].strip(),conf["keyb_id"][i].strip(),
            conf["transmit_address"][i].strip(),conf["manager"].strip(),conf["transmit_address"][i].strip())    
        saveFile("jobs/job_ocr_oracle_{}.toml".format(str(i+1)), ocr_job)

        data_job = dataFetcherJob.replace("-ope-",conf["dr"].strip()).replace("-tra-",conf["transmit_address"][i].strip())
        saveFile("jobs/job_data_oracle_{}.toml".format(str(i+1)), data_job)

    boot_job = bootsrap_job.format(conf["ocr"].strip(),conf["boot_kb"].strip())
